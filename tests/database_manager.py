...
def test_check_data_base_existence_positive():
    ...
    # THEN
    assert test_database.exists is True
...
def test_check_data_base_existence_negative():
    ...
    test_database.check_database_existence()
    # THEN
    assert test_database.exists is False
...
def test_get_materials_to_review():
    """Checks if method returns correct list of materials where difference between today
    and last review date is equal or greater than standard - 3 days. Used wrapper to
    freeze variable indicates today in testcase"""
...
def test_get_materials_to_review_shortened_days_interval():
    """Checks if method returns correct list of materials where difference between today
    and last review date is equal or greater than indicated 2 days. Used wrapper to freeze
    variable indicates today in testcase"""
...
def test_change_current_stock_correct_run():
    """Checks if method correctly change stock for provided, existing
    material and automatically set current date"""

    # GIVEN
    expected_materials_return = [
        (1, '22REW', 345721, 300, 7.89, '2022-04-21', 'autoadmfactor@gmail.com'),
        (2, '32REW', 345718, 2000, 4.2, '2022-04-18', 'adampolakfactor@gmail.com'),
        (3, 'BYSE', 345719, 10000, 3, '2022-04-17', 'autoadmfactor@gmail.com'),
        (4, 'OILB', 345729, 1740, 11.4, '2022-04-20', 'adampolakfactor@gmail.com')]
...
def test_change_current_stock_text_as_sku():
    """Checks if provided sku as string is handled as exception and does not
...
def test_change_current_stock_sku_ok_qty_wrong():
    """Checks if wrongly provided quantity as input does not change anything in database
    as expected"""
...
def test_change_current_stock_sku_ok_qty_expected_result_wrong():
    """Checks if wrongly provided quantity is saved in database. Scenario considers
    wrong attitude that quantity is changed. Thus expected result is not equal to actual"""
